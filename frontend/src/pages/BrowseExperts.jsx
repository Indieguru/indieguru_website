...
export default function BrowseExperts() {
  ...
  const fetchExperts = async () => {
    try {
      const response = await axiosInstance.get('/expert/all');
  ...
  const filteredExperts = experts.filter(expert => {
    ...
    const matchesExperience = !filters.experience || 
      expert.experience >= parseInt(filters.experience);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-center mb-8">Find Your Expert</h1>
        ...
        {filteredExperts.map((expert) => (
          <div key={expert._id} className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center mb-4">
              <img
                src={expert.profileImage || '/placeholder-user.jpg'}
                alt={expert.name}
                className="w-16 h-16 rounded-full object-cover mr-4"
              />
              <div>
                <h3 className="text-xl font-semibold">{expert.name}</h3>
                <p className="text-gray-600">{expert.experience} years experience</p>
              </div>
            </div>
